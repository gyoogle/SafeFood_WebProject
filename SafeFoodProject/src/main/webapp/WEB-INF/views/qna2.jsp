	...
	function addBoard2(){
        ...
        var boardresult = Vue.component("board-result", { // board-result : component 이름
            ...
            methods : {
                allBoard : function(){
                    axios
                    .get('http://localhost:9082/articles')        // 요청
                    .then(response=>{
                    	console.log("???");
            	...
            	search : function() {
		        	...
		        	if (this.option == ''){
		        		...
		        		axios
		                .get('http://localhost:9082/articles/title/' + this.searchContent)        // 요청
		                .then(response=>(this.result=response.data))
		        	}
		        	
		        	else if (this.option == '2'){
		        		axios
		                .get('http://localhost:9082/articles/name/' + this.searchContent)        // 요청
		                .then(response=>(this.result=response.data))
		        	}
            	}
            }
            ...
            methods : {
            	search : function() {
		        	if (this.$route.params.option == '1'){
		        		...
		        		console.log(this.searchContent);
		        		axios
		                .get('http://localhost:9082/articles/title/' + this.$route.params.searchContent)        // 요청
		                .then(response=>(this.result=response.data))
		        	}
		        		...
		        		console.log('2xxxx');
		        		axios
		                .get('http://localhost:9082/articles/name/' + this.$route.params.searchContent)        // 요청
		                .then(response=>(this.result=response.data))
		        	}
            	}
            }
        ...
        var addboard = Vue.component('addboard', {
        	...
        	methods : {
        		...
        		addBoard : function(){
                	axios
                    .post('http://localhost:9082/articles', {pass:this.pass, name:"${sessionScope.id}", title:this.title, content:this.content})        // 요청
                    .then(response=>{
                    	console.log("실행됨")
        ...
        var updateboard = Vue.component('updateboard', {
        	...
        	methods : {
                ...
                updateBoard : function(){
                	axios
                    .put('http://localhost:9082/articles', {num:this.num, title:this.title, content:this.content})        // 요청
                    .then(response=>{
                    	alert("수정이 완료되었습니다.");
//                     	this.allBoard();
                    	this.$router.push('/');
        ...
        var viewboard = Vue.component('viewboard', {
        	...
        	methods : {
        		passwordConfirm : function(){
        			...
        			axios
        			.get('http://localhost:9082/articles/passConfirm/' + this.num + "/" + this.passNow)
        			.then(response=>{
                ...
                selectBoard : function(num){
                	axios
                    .get('http://localhost:9082/articles/'+ num)        // 요청
                    .then(response=>{
...
//         				console.log("결과2 : " + this.passNowResult)
	        			...
	        			if (this.passNowResult.result == 'correct'){
	        				axios
	                        .delete('http://localhost:9082/articles/'+ this.num)        // 요청
	                        .then(response=>{
	                        	alert("삭제가 완료되었습니다.")
	                        	this.$router.push('/');
	                        	this.allBoard()
